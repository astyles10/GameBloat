...
// Note this is also the priority list for interrupts; VBlank being the highest, Joypad being lowest
enum interruptBits
{
...
const int interruptBits[] = {VBLANK_INTERRUPT, LCD_INTERRUPT, SERIAL_INTERRUPT, TIMER_INTERRUPT, JOYPAD_INTERRUPT};

struct interruptRegisters interruptRegisters;

unsigned char validateInterrupt(unsigned char interruptBit)
{
  for (int bit = 0; bit < (sizeof(interruptBits) / sizeof(*interruptBits)); bit++)
  {
    if (bit == interruptBit)
    {
      return 1;
...
void interruptStep(void)
{
  ...
  {
    unsigned char flags = (interruptRegisters.enable & interruptRegisters.request);
    if (flags > 0)
    {
