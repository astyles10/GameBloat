...
enum MBC3_AccessModes { RAM_MODE = 0, RTC_MODE = 1 };
...
enum TimeAsSeconds { SECOND = 1, MINUTE = 60, HOUR = 3600, DAY = 86400 };
...
const unsigned int MBC3_WriteByte(const unsigned short memAddr,
  ...
  if (address <= 0x1FFF) {
    ...
    // Latch Clock Data sequence Write 0x00, then 0x01 to latch to RTC
    // registers.
...
void updateBaseTimestamp(unsigned int timeUnitAsSeconds,
                         unsigned short timeUnit, unsigned char writeValue) {
  time_t oldTimestamp = determineTimestamp();
  time_t oldTimeUnit =
      ((oldTimestamp - baseTimestamp) / timeUnitAsSeconds) % timeUnit;
...
void latchRTC(void) {
  /*
    ...
    becomes latched again, by repeating the write 00h->01h procedure. This is
    supposed for <reading> from the RTC registers. It is proof to read the
    latched (frozen) time from the RTC registers, while the clock itself
 ...
 */
  ...
  // Determine day overflow - total 9 bits (day counter lower + upper) count
  // days in range 0 - 511 Day counter overflow sets when the 9 bit day counter
  // overflows, which remains until program resets it
  ...
  Clock.lowerDayCounter =
      (unsigned char)((elapsedTime / DAY) & DAYS_LOWER_UNIT);

  Clock.upperDayCounter &= ~(dayCounterMSB);
  Clock.upperDayCounter =
      (unsigned char)(((elapsedTime / DAY) & DAYS_UPPER_UNIT) >> 8);
